...
- Pre-flight schema validation (`schema.Validate`)
- Graceful shutdown via SIGINT with partial dataset output
- Per-table error recovery with `Dataset.Errors`
- Structured leveled logging (INFO/WARN/ERROR/DEBUG)
- YAML config file support (`--config`, `--init-config`)
